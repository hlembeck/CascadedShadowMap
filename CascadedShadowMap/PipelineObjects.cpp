...
void PipelineObjects::LoadPlanetRenderPSO() {
    D3D12_INPUT_ELEMENT_DESC elemDescs[2] = {
            {"POSITION", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
    };
    D3D12_INPUT_LAYOUT_DESC layoutDesc = {
    ...
    ComPtr<ID3DBlob> pixelShader;
    UINT compileFlags = 0;
    ThrowIfFailed(D3DCompileFromFile(L"PlanetRender.hlsl", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "VS", "vs_5_1", compileFlags, 0, &vertexShader, NULL));
    ThrowIfFailed(D3DCompileFromFile(L"PlanetRender.hlsl", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "PS", "ps_5_1", compileFlags, 0, &pixelShader, NULL));
    ...
    psoDesc.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM;
    psoDesc.DSVFormat = DXGI_FORMAT_D32_FLOAT;
    ThrowIfFailed(m_device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_planetRenderPSO)));
}

void PipelineObjects::LoadPlanetTilerPSO() {
    ComPtr<ID3DBlob> cs;
    UINT compileFlags = 0;
    ThrowIfFailed(D3DCompileFromFile(L"PlanetTiler.hlsl", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "CS", "cs_5_1", compileFlags, 0, &cs, NULL));
    ...
    ThrowIfFailed(m_device->CreateComputePipelineState(&psoDesc, IID_PPV_ARGS(&m_planetTilerPSO)));
}

void PipelineObjects::LoadIntersectionPSO() {
    ComPtr<ID3DBlob> cs;
    UINT compileFlags = 0;
    ThrowIfFailed(D3DCompileFromFile(L"PlanetIntersection.hlsl", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "CS", "cs_5_1", compileFlags, 0, &cs, NULL));
    ...
    ThrowIfFailed(m_device->CreateComputePipelineState(&psoDesc, IID_PPV_ARGS(&m_intersectionPSO)));
}
...
void PipelineObjects::OnInit() {
    ...
    m_planetRenderPSO->SetName(L"Planet Render PSO");
    m_planetTilerPSO->SetName(L"Planet Tiler PSO");
    m_intersectionPSO->SetName(L"Intersection PSO");
}