...
void CelestialManager::GenerateInitialCelestialBodies() {
	//test
	//_celestialBodies[m_numBodies++] = new Planet((CommandListAllocatorPair*)this, 1.0f, {0.0f,0.0f,0.0f});
}
...
void CelestialManager::Init(BoundingFrustum viewFrustum) {
	CommandListAllocatorPair::Init();
...
void CelestialManager::Render(ID3D12GraphicsCommandList* commandList, FrustumRays rays) {
	m_celestialBodies[0]->FillRenderCmdList(rays.o,commandList);
