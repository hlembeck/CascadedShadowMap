...
cbuffer BlockDepth : register(b3) {
	uint blockDepth;
	float3 invSquareAxisLengths;
};

float Ellipsoid(float3 pos) {
	return invSquareAxisLengths.x * pos.x * pos.x + invSquareAxisLengths.y * pos.y * pos.y + invSquareAxisLengths.z * pos.z * pos.z - 1;
}
